    ...
    const handleLogin = async () => {
      try {
        ...
        if (data.success) {
          ...
          alert(data.error); // ✅ Now it shows: "No account with that email exists." or "Incorrect password."
        }
      } catch (error) {
            ...
            <div className='h-screen min-w-[400px] w-[45%] pt-[2%] flex-col items-center justify-center'>
                ...
                <div className=' mx-auto h-[70%] w-[80%] flex flex-col gap-[10px] mt-[2%]'>
                    <div className='flex-col  justify-end '>
                        ...
                        <input onChange={e => setEmail(e.target.value) } type="email" className='w-full h-[40px] bg-white border border-gray-500 border-1 px-2 focus:border-[#FFCC66] focus:ring-1 focus:ring-[#FFCC66] outline-none shadow-lg' />
                    </div>
                    <div>
                        ...
                        <div className="relative w-full">
                          ...
                          <img
                          src={showPassword ? Hide : Show} // your image paths
                    ...
                    <button onClick={handleLogin} className='font-outfit mt-[5%] bg-[#FFCC66] px-[50px] py-[10px] mx-auto font-bold shadow-lg hover:bg-white transition-colors duration-300 mb-[2%]'>Login</button>
                    <h1 className='font-outfit text-white text-center mt-[10%] text-medium md:text-xl'>Don't have an account?  <Link to='/' className='text-[#FFCC66] underline hover:text-white'>Sign up</Link></h1>
                </div>
            </div>
        </div>
    )
